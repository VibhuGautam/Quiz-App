const qBank = [
  ...
  {
    question: "What does a funambuanswers walk on?",
...
var result=qBank.map((x)=>{
  var ll=x.answers[0];
  x.answers.shift();
  
  var ii=Math.floor(Math.random() * (x.answers.length));
  
  x.answers.splice(ii, 0, ll);
  return x;
});

export default (n = 5) =>
  Promise.resolve(result.sort(() => 0.5 - Math.random()).slice(0, n));
